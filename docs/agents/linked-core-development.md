...
| ----------------------------- | ------- | ------------------------------------------ |
| `composer.json`               | yes     | the real published dependency, currently `^0.10`. Constrain to a patch (`^0.3.1`, never `^0.3`) whenever a version has been published from a stale commit and must be skipped |
